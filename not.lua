...
export type Dropdown = {
	...
	Values: { string },
	Multi: boolean,
	Callback: (Value: string | { string }) -> any?,
}
...
function Compkiller:_GetIcon(name: string, font_aws): string
	if Compkiller.SecureMode then
		...
		if font_aws then
			...
			AssetId = Compkiller.Lucide["lucide-" .. tostring(name)]
				or Compkiller.Lucide[name]
				or Compkiller.Lucide[tostring(name)]
				or Compkiller.FontAwesome[name]
				or name
		end
	...
	return Compkiller.Lucide["lucide-" .. tostring(name)]
		or Compkiller.Lucide[name]
		or Compkiller.Lucide[tostring(name)]
		or Compkiller.FontAwesome[name]
		or name
...
function Compkiller:GetCalculatePosition(
	planePos: number,
	planeNormal: number,
	rayOrigin: number,
	rayDirection: number
...
function Compkiller:Drag(InputFrame: Frame, MoveFrame: Frame, Speed: number)
	...
	InputFrame.InputBegan:Connect(function(input)
		if
			...
			and #Compkiller.DragBlacklist <= 0
		then
			dragToggle = true
	...
	UserInputService.InputChanged:Connect(function(input)
		if
			...
			or input.UserInputType == Enum.UserInputType.Touch and #Compkiller.DragBlacklist <= 0
		then
			if dragToggle then
...
function Compkiller:_KeybindHandler(
	Parent: Frame,
	ObjectType: string,
	...
	function Args:AddOption()
		...
		UserInputService.InputBegan:Connect(function(Input)
			...
			then
				if
					Toggl
					and not Compkiller:_IsMouseOverFrame(ExtractElement)
					and not Compkiller:_IsMouseOverFrame(Element)
				then
					ToggleUI(false)
				end
			end
...
function Compkiller:_LoadDropdown(BaseParent: TextButton, Callback: () -> any)
	...
	local NormalizeMultiValue = function(value)
		...
		for option, enabled in next, value do
			if enabled then
		...
		for i, v in next, Values do
			...
			if IsMulti then
				...
				Compkiller:_Input(bth.DropdownItem, function()
					...
					Callback(NormalizeMultiValue(DataFrame))
				end)
			else
				Compkiller:_Input(bth.DropdownItem, function()
					if DataFrame then
...
function Compkiller:_LoadElement(Parent: Frame, EnabledLine: boolean, Signal, DisableStackKeybind)
	...
	function Args:AddDropdown(Config: Dropdown)
		...
		function Args:SetValue(Value)
			Config.Default = (Config.Multi and NormalizeMultiValue(Value)) or Value
		...
		function Args:GetValue()
			return (Config.Multi and NormalizeMultiValue(Config.Default)) or Config.Default
		end
	...
	function WindowArgs:DrawConfig(Configuration: TabConfigManager, Internal)
		...
		function TabArgs:_DrawConfig()
			...
			function ConfigButton:SetInfo(Author, ConfigName)
				local R, G, B =
					tostring(math.floor(Compkiller.Colors.Highlight.R * 255)),
					tostring(math.floor(Compkiller.Colors.Highlight.G * 255)),
					tostring(math.floor(Compkiller.Colors.Highlight.B * 255))
